...
<body onload="fade();" style="background-image: url('imgs/loading_bg_1.gif');">
  ...
  <?php
    if (isset($_SESSION["username"])){
      $token = uniqid();
      echo "<form action='loading.php?token=$token' method='post' class='x needs-validation' novalidate>";
    } else {
      echo "<form action='includes/util.inc.php' method='post' class='x needs-validation' novalidate>";
    }
  ?>
    <?php
      if (isset($_SESSION["username"])){
    ...
    <div class="play">
        ...
        <?php
          if (isset($_SESSION["username"])){
            echo "<button type='submit' name='submit_fight' class='btn btn-danger'><strong>FIGHT</strong></button>";
...
<form action="includes/util.inc.php" class="search" method="post">
...
<div class="page_down_2">
  ...
  <!-- Contact us -->
  <form action="includes/util.inc.php" method="post" class="z needs-validation" novalidate>
    <div class="mb-3">
